...
**别拿「精确字符串」当支点。今天同一个毛病栽了三次。**

三次长得不一样，根子是一个：**支点选得太脆，环境稍微一变就整条失配，而且往往不报错，是静悄悄什么都没发生。**

| 栽在哪 | 脆在哪 | 换成什么 |
|---|---|---|
| CSS `:first-of-type` | 它按**标签类型**算，不按 class 算。首页 DOM 一变，整条规则匹配不到任何元素 | 别用它定位某个 class 的"第一个"。要么给确切选择器，要么让规则对全体生效 |
| `lace_band` 当分隔条 | 它是从信纸上裁的**矩形**（带纸底），`repeat-x` 平铺出来是一排砖，接缝全露 | 矩形裁片只能当**背景**，不能当**线**。要线就用 CSS 画，要花就只在中间放一个去背的件 |
| 补丁锚点用精确字符串 | 抄的是补丁**源码**里那一行，空格对齐跟线上那份不一样；函数签名也可能被别的补丁加过参数 | 整行正则 + 找不到时把附近的行打出来（见 `add-jerkoff.js`） |

**写补丁锚点的三条：**

1. **用正则，不用字符串。** `/^[ \t]*\{[ \t]*s:[ \t]*n\('sensitivity'\)[^\n]*?想让她碰我[^\n]*$/m`
   —— 认内容，不认空格。
2. **不拿函数签名当锚点。** 别的补丁随时会给它加参数。要往函数体开头插东西，
   就用 `/function\s+名字\s*\([^)]*\)\s*\{/` 捕获，然后在捕获到的原文后面追加；
   插进去的代码**自己去拿它要的数据**，别依赖原函数怎么解构。
3. **锚点找不到必须打印附近的行。** 一行 `console.error` 的事，
   能省掉一整轮"她跑一次、报一句、我猜一次"。今天为这个多跑了两轮。

**顺带一条验证纪律：** 补丁写完要对着**几种不同风格的 fixture** 各跑一遍
（空格对齐的 / 压缩的 / tab 缩进的 / 函数带参数的）。add-jerkoff 是这么测的，四种全过才推。
